...
class bottleneck_1(nn.Module):
    def __init__(self,input_dim=3, output_dim = 6):
        ...
        self.cnn1 = nn.Conv1d(in_channels=input_dim, out_channels=input_dim*2, kernel_size=1,stride = 1,padding = 0)
        self.batchnorm1 = nn.BatchNorm1d(input_dim*2)
        self.act1 = nn.ReLU()

        self.cnn2 = nn.Conv1d(in_channels=input_dim*2, out_channels=input_dim*2, kernel_size=3,stride = 1,padding = 1, padding_mode= 'replicate')
        self.batchnorm2 = nn.BatchNorm1d(input_dim*2)
        self.act2 = nn.ReLU()

        self.cnn3 = nn.Conv1d(in_channels=input_dim*2, out_channels=output_dim, kernel_size=1,stride = 1,padding = 0)
        self.batchnorm3 = nn.BatchNorm1d(output_dim)
        self.act3 = nn.ReLU()

        self.shortcut = nn.Sequential(nn.Conv1d(in_channels=input_dim, out_channels=output_dim, kernel_size=1,stride = 1,padding = 0),
...
class bottleneck_2(nn.Module):
    def __init__(self,input_dim=3, output_dim = 3):
        ...
        self.cnn3 = nn.Conv1d(in_channels=input_dim*2, out_channels=input_dim, kernel_size=1,stride = 1,padding = 0)
