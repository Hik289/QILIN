...
class transformer_sxy_1(BasicModule):
    def __init__(self, input_dim = 3,nhead = 1, num_layers = 1, dim_feedforward = 2048,num_encoders = 1, 
                            batch_size = 1200, resnet = False,dropout = 0.1):
        ...
        self.cls_token = torch.ones(batch_size, self.d_model).cuda()
        self.sep_token = torch.ones(batch_size, self.d_model).cuda()
    ...
    def forward(self,x):
        if x.shape[0] != self.sep_token.shape[0]:
            self.cls_token = torch.ones(x.shape[0], self.d_model).cuda()
            self.sep_token = torch.ones(x.shape[0], self.d_model).cuda()            
...
class transformer_sxy_2(BasicModule):
    def __init__(self, input_dim = 3,nhead = 1, num_layers = 1, dim_feedforward = 2048,num_encoders = 1, 
                            batch_size = 1200, resnet = False,dropout = 0.1):
        super(transformer_sxy_2,self).__init__()
        self.model_name = 'transformer_sxy_2: BERT:hidden pooling'
